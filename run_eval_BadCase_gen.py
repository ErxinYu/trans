...
def main(args):
    ...
    bad_case_dir = "/home/yex/LESS/data/eval/mmlu/bad_case/"
    if not os.path.exists(bad_case_dir):
        os.makedirs(bad_case_dir)
    ...
    for subject in tqdm(subjects, desc=f"Evaluating subjects: "):
        ...
        # Save bad cases if any
        bad_cases = test_df[test_df["correct"] == False]
        if not bad_cases.empty:
            original_test_df = pd.read_csv(
                os.path.join(args.data_dir, "dev", subject + "_dev.csv"), header=None
            )
            bad_case_indices = bad_cases.index
            bad_cases_original = original_test_df.loc[bad_case_indices]
            bad_cases_original.to_csv(
                os.path.join(
                    bad_case_dir, "{}_dev.csv".format(subject)
                ),
                index=None,
                header=False
            )
